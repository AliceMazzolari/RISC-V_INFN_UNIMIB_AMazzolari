...
#define SR_EOC (1 << 5) /* end of conversion signal */
#define SR_EOS (1 << 6) /* end of sequence */
#define SR_BUSY (1 << 7) /* ADC busy signal */

#define GIER_EN (1 << 31) /* Interrupt enable*/
#define IPISR_EOS (1 << 4) /* Interrupt endo of sequence */

#define SINGLE_CHANNEL 0x3000
#define TEMP 0xFFE0
#define VPVN 0x3
#define CONT 0x200
...
void kputreg() {
    ...
    xregs_hard_macro->configuration0 |= CONT;
    xregs_hard_macro->configuration1 &= ~0xF000;
    xregs_hard_macro->configuration1 |= SINGLE_CHANNEL;
    xregs_hard_macro->configuration0 &= TEMP;
    xregs_hard_macro->configuration0 |= VPVN;
    
    //while (uregs->status & SR_TX_FIFO_FULL) {}
    //uregs->tx_fifo = xregs_hard_macro->configuration0 & 0xFF;
    //while (uregs->status & SR_TX_FIFO_FULL) {}
    //uregs->tx_fifo = (xregs_hard_macro->configuration0 >> 8) & 0xFF;
    //while (uregs->status & SR_TX_FIFO_FULL) {}
    //uregs->tx_fifo = (xregs_hard_macro->configuration0 >> 16) & 0xFF;
    //while (uregs->status & SR_TX_FIFO_FULL) {}
    //uregs->tx_fifo = (xregs_hard_macro->configuration0 >> 24) & 0xFF;
    
    uint16_t t[100000];
    
    for (int i=0; i<100000; i++) {
    	while (!(xregs_local->SR & SR_EOC)) {}
    	t[i] = xregs_hard_macro->VpVn;
    }
    
    for (int i=0; i<100000; i++) {
    	while (uregs->status & SR_TX_FIFO_FULL) {}
    	uregs->tx_fifo = (t[i] >> 4) & 0xff;
    	while (uregs->status & SR_TX_FIFO_FULL) {}
    	uregs->tx_fifo = (t[i] >> 12) & 0xf;
    }
}
